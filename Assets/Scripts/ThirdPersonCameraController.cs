...
public class ThirdPersonCameraController : MonoBehaviour
{
    ...
    public float rotationSmoothTime = 0.15f;            //회전 부드러움
    public float heightSmoothTiime = 0.2f;              //Y축 높이 전용 부드러움
    ...
    private void Start()
    {
        ...
        if (cameraTarget == null)                       //카메라의 타깃을 생성
        {
    ...
    void PositionCamera()
    {
        ...
        //y축을 별도로 부드럽게 처리
        smoothedTargetY = Mathf.SmoothDamp(smoothedTargetY, targetPos.y, ref targetYVelocity, heightSmoothTiime);
        ...
        //카메라 위치 부드럽게 이동
        transform.position = Vector3.SmoothDamp(transform.position, desiredPosition, ref positionVelocity, heightSmoothTiime);
